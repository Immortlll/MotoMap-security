...
async def filter_content_async(content: str, user_id: str = "anonymous") -> FilterResult:
    """异步过滤内容"""
...
async def filter_content_endpoint(request: FilterRequest):
    ...
    try:
        ...
        if request.return_details:
            response.detected_patterns = result.detected_patterns
            response.original_length = len(request.content)
            response.filtered_length = len(result.filtered_content)
...
@app.post("/batch_filter", response_model=BatchFilterResponse)
...
@app.post("/check_security", response_model=SecurityCheckResponse)
